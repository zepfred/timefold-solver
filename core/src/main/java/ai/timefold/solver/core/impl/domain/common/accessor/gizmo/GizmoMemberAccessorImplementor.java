...
public final class GizmoMemberAccessorImplementor {
    ...
    private static Class<? extends AbstractGizmoMemberAccessor> getCorrectSuperclass(GizmoMemberInfo memberInfo) {
        ...
        memberInfo.descriptor().whenIsMethod(
                method -> supportsSetter.set(memberInfo.descriptor().getSetter().isPresent() || memberInfo.acceptParameter()));
     ...
     * @param acceptParameter A flag that indicates if the method accepts parameter
    ...
    static MemberAccessor createAccessorFor(Member member, Class<? extends Annotation> annotationClass,
        ...
        GizmoMemberInfo memberInfo =
                new GizmoMemberInfo(new GizmoMemberDescriptor(member), returnTypeRequired, acceptParameter, annotationClass);
    ...
    private static void createConstructor(ClassCreator classCreator, GizmoMemberInfo memberInfo) {
        ...
        memberInfo.descriptor().whenMetadataIsOnMethod(md -> {
            ...
            String[] types = md.getParameterTypes();
            if (types.length > 1) {
                throw new IllegalStateException("The method %s has %d parameters, it is only allowed to have one parameter"
                        .formatted(md.getName(), types.length));
            }
            ResultHandle method;
            if (memberInfo.acceptParameter() && types.length > 0) {
                ResultHandle params = methodCreator.newArray(Class.class, 1);
                ResultHandle paramType =
                        methodCreator.loadClass(org.objectweb.asm.Type.getType(types[0]).getClassName().replace('.', '/'));
                methodCreator.writeArrayValue(params, 0, paramType);
                method = methodCreator.invokeVirtualMethod(MethodDescriptor.ofMethod(Class.class, "getDeclaredMethod",
                        Method.class, String.class, Class[].class),
                        declaringClass, name, params);
            } else {
                method = methodCreator.invokeVirtualMethod(MethodDescriptor.ofMethod(Class.class, "getDeclaredMethod",
                        Method.class, String.class, Class[].class),
                        declaringClass, name,
     ...
     */
    private static void assertIsGoodMethod(MethodDescriptor method, boolean returnTypeRequired, boolean acceptParameter) {
        // V = void return type
        // Z = primitive boolean return type
        String methodName = method.getName();
        if (!acceptParameter && method.getParameterTypes().length != 0) {
    ...
    private static void createGetName(ClassCreator classCreator, GizmoMemberInfo memberInfo) {
        ...
        memberInfo.descriptor().whenIsMethod(method -> {
            ...
            if (annotationClass == null) {
                assertIsGoodMethod(method, memberInfo.returnTypeRequired(), memberInfo.acceptParameter());
            } else {
    ...
    private static void createExecuteGetter(ClassCreator classCreator, GizmoMemberInfo memberInfo) {
        ...
        if (memberInfo.acceptParameter()) {
            // When it is a method,
            // and the corresponding setter method exists,
            // it indicates that the method execution will require a parameter.
            // The build pattern is used by the CascadingUpdateShadowVariable target method,
            // and the setter method should be instead
